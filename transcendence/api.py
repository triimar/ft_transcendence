...
def guest_login(request):
    guest = shortuuid.ShortUUID().random(length=6)
    # avatar = assign_random_avatar()
    # color = assign_random_background_color()
    # save_user_cache(guest, avatar, color, guest=True)
    return JsonResponse({'guest_login': guest})
...
def oauth_callback(request):
    ...
    user_data_response = requests.get(
        ...
        headers = {'Authorization': f'Bearer {access_token_response.json().get('access_token')}'}
    )
